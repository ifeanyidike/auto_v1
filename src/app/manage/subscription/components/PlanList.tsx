...
const PlanList = (props: Props) => {
  ...
  const [itemDetail, setItemDetail] = useState<PlanByMerchant | null>(null);
  const [filterData, setFilterData] = useState<Partial<Filter>>();
  const [planList, setPlanList] = useState<PlanByMerchant[]>(props.plans);

  const handleFilter = () => {
    const newPlanList = props.plans.filter(plan => {
      let condition = true;
      if (filterData?.brand) {
        condition &&= plan.autoBrand === filterData.brand;
      }
      if (filterData?.code) {
        condition &&= plan.code === filterData.code;
      }

      if (filterData?.interval) {
        condition &&= plan.interval === filterData.interval;
      }

      if (filterData?.planName) {
        const planName = `${plan.merchantService?.service?.title} - ${plan.autoBrand} - ${plan.interval}`;
        condition &&= planName === filterData.planName;
      }
      return condition;
    });
    ...
    return (
      <>
        ...
        >
          <button
            onClick={() => {
              const item = planList.find(plan => plan.id === popupOpen.id);
      ...
      >
        ...
        <button className="flex gap-1 mb-2 flex-col w-full px-3 pt-2">
          ...
          <TextInput
            suffixSign={<SearchIcon />}
            name="brand"
            placeholder="Search by auto brand"
            customStyle="!py-2 !rounded-l-full text-xs"
            customSuffixStyle="!rounded-r-full"
            getValue={(value: string) => {
              const newFilterData = { ...(filterData || {}) };
              newFilterData.brand = value;
              setFilterData(newFilterData);
            }}
          />
        </button>
        ...
        <button className="flex mb-2 gap-2 w-full flex-col px-3 pt-2">
          ...
          <DropdownSelect
            data={[
              ...
              { caption: 'Quarterly', value: 'quarterly' },
  ...
  return (
    <div className="relative mt-8">
      ...
      {itemDetail && (
        <PlanDetail onClose={() => setItemDetail(null)} item={itemDetail} />
      )}
      <div className="border-2 relative border-white rounded-xl mt-3 overflow-auto bg-white">
        {[heading, ...planList].map((data, index) => (
