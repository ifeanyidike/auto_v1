...
import ProductPane from '../../manage/product/components/ProductPane';
...
import BellIcon from '~/commons/icons/BellIcon';
...
import { updateBookingStatus } from '../../manage/booking/action';
import { SnackbarProvider, enqueueSnackbar } from 'notistack';
import LoaderOne from '~/components/LoaderOne';

type Props = {
  item: bookingByUserItem;
  bookingList: bookingByUserItem[];
  setBookingList: React.Dispatch<React.SetStateAction<bookingByUserItem[]>>;
  onClose: () => void;
};

const BookingDetail = (props: Props) => {
  const [item, setItem] = useState<bookingByUserItem>(props.item);
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col gap-7 h-[calc(100%-56px)] overflow-auto">
          <ProductPane paneTitle="Merchant Details" initExpanded>
              ...
              <div className="grid grid-cols-3 place-items-center gap-5 max-lg:grid-cols-2 max-md:grid-cols-1">
                ...
                <div className="flex flex-col text-sm gap-1">
                  ...
                  <div className="flex gap-1">
                    <PhoneIcon />
                    <p className={`${dmSans.className}`}>
                      {item.merchant?.phoneNo ?? 'Not provided'}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col text-sm gap-1">
                  <small className="text-gray-400">Workshop Address</small>

                  <div className="flex gap-1">
                    <HomeIcon />
                    <p className={`${dmSans.className}`}>
                      {item.merchant?.address ?? 'Not provided'}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col text-sm gap-1">
                  <small className="text-gray-400">Workshop Caption</small>

                  <div className="flex gap-1">
                    <BellIcon />
                    <p className={`${dmSans.className}`}>
                      {item.merchant?.caption ?? 'Not provided'}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col text-sm gap-1">
                  ...
                  <div className="flex gap-1">
                    <ProfileIcon />
                    <p className={`${dmSans.className}`}>
                      {item.merchant?.shortDescription ?? 'Not provided'}
                    </p>
                  </div>
                </div>
              </div>
