...
const UserBookingList = (props: Props) => {
  ...
  const [itemDetail, setItemDetail] = useState<bookingByUserItem | null>(null);
  const [filterData, setFilterData] = useState<Partial<Filter>>();
  const [bookingList, setBookingList] = useState<bookingByUserItem[]>(
    props.bookings || []
  );
  ...
  const heading = { id: 'table_header' } as unknown as bookingByUserItem;
  const renderPopup = () => {
    if (popupOpen === null) {
      return null;
    }
